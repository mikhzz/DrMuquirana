...
public class PainelListagemDespesas extends JPanel {
		...
		protected void consultarDespesas() {
			...
			ArrayList<DespesaVO> despesas = controller.consultarTodasDespesasController();
		...
		protected void atualizarTabelaDespesas(ArrayList<DespesaVO> despesas) {
			...
			for(DespesaVO desp: despesas) {
				...
				novaLinha[3] = desp.getDtVencimento().format(formatadorDeData);

				if(desp.getDtPagamento() != null) {
					novaLinha[4] = desp.getDtPagamento().format(formatadorDeData);
				}
